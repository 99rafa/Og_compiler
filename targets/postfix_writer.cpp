...
void og::postfix_writer::do_variable_declaration_node(og::variable_declaration_node *const node, int lvl) {
  ...
  if (_inFunctionBody && node->initializer() && 
      node->is_typed(cdk::TYPE_STRUCT) && node->identifiers().size() != 1);
  ...
  if (_inFunctionBody) {
    ...
    if (node->initializer()) {
      if (node->is_typed(cdk::TYPE_STRUCT) && node->identifiers().size() != 1) {
          ...
          auto initializer = dynamic_cast<og::tuple_node *>(node->initializer());

          if( initializer->arguments()->size() == 1){
            _inTuple = true;
            initializer->arguments()->node(0)->accept(this,lvl);
            _inTuple = false;
            int cont = 0;
            for(size_t i = 0; i < node->identifiers().size(); i++){
              auto type = cdk::structured_type_cast(initializer->type())->component(i);
              _offset-= type->size();
              syms.at(i)->set_offset(_offset);
              _pf.DUP32();
              _pf.INT(cont);
              _pf.ADD();
            ...
            for (size_t i = 0; i < initializer->arguments()->size(); i++) {
              cdk::expression_node * var = dynamic_cast<cdk::expression_node *>(initializer->arguments()->node(i));
                if (var->is_typed(cdk::TYPE_INT) || var->is_typed(cdk::TYPE_STRING)
                || var->is_typed(cdk::TYPE_POINTER)) {
                  initializer->arguments()->node(i)->accept(this,lvl);
                  _offset-= var->type()->size();
                  syms.at(i)->set_offset(_offset);
                  _pf.LOCAL(_offset);
                  _pf.STINT();
              } else if (var->is_typed(cdk::TYPE_DOUBLE)) {
                initializer->arguments()->node(i)->accept(this,lvl);
                _offset-= var->type()->size();
                syms.at(i)->set_offset(_offset);
                _pf.LOCAL(_offset);
                _pf.STDOUBLE();
              } else if(var->is_typed(cdk::TYPE_STRUCT)){
                _offset-= var->type()->size();
                syms.at(i)->set_offset(_offset);
                _inTuple = true;
                initializer->arguments()->node(i)->accept(this,lvl);
...
void og::postfix_writer::do_tuple_index_node(og::tuple_index_node *const node, int lvl) {
  ...
  for (int i = 0; i <  node->index() - 1 ; i++) {
      alloc += cdk::structured_type_cast(node->base()->type())->component(i)->size();
  }
...
void og::postfix_writer::do_tuple_node(og::tuple_node *const node, int lvl) {
  //TODO tuples em tuples
  // if(_inReturn){
  //   _pf.INT(node->type()->size());
  //   _pf.ALLOC();
  //   _pf.SP();
  // } else 
  if( (_inFunctionBody && _inTuple) || (_return && node->arguments()->size() != 1)){
    int off = 0;
    if(node->arguments()->size() == 1){
      node->arguments()->node(0)->accept(this, lvl +2);
      if(_trash){
        _trash = false;
        _pf.TRASH(4);
      }
        ...
        for(size_t i = 0; i < node->arguments()->size(); i++){
          auto ex = dynamic_cast<cdk::expression_node*>(node->arguments()->node(i));
          bool cast =_function->is_typed(cdk::TYPE_STRUCT) && ex->is_typed(cdk::TYPE_INT) && cdk::structured_type_cast(_function->type())->component(i)->name() == cdk::TYPE_DOUBLE;
          
          _pf.DUP32(); // dup sp
          if(ex->is_typed(cdk::TYPE_DOUBLE) || cast)
            _pf.DUP32();

          node->arguments()->node(i)->accept(this, lvl +2);
          if(cast){
            _pf.I2D();
            ex->type(cdk::make_primitive_type(8,cdk::TYPE_DOUBLE));
          }
          ...
          if(ex->is_typed(cdk::TYPE_DOUBLE)){
            _pf.SWAP64();
            _pf.TRASH(4);
          }
          else
  ...
  } else node->arguments()->accept(this, lvl + 2);  
