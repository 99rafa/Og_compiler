...
void og::type_checker::processBinaryPIDExpression(cdk::binary_operation_node *const node, int lvl) {
  ...
  if (node->left()->is_typed(cdk::TYPE_DOUBLE) && node->right()->is_typed(cdk::TYPE_DOUBLE)) {
      ...
      node->type(make_reference_type(4,cdk::reference_type_cast(node->left()->type())->referenced()));
  }
  else if (node->left()->is_typed(cdk::TYPE_INT) && node->right()->is_typed(cdk::TYPE_POINTER)) {
      node->type(make_reference_type(4,cdk::reference_type_cast(node->right()->type())->referenced()));
  }
...
void og::type_checker::do_print_node(og::print_node *const node, int lvl) {
   ...
   for(size_t i = 0; i < node->arguments()->size(); i++){
      ...
      if( n->is_typed(cdk::TYPE_UNSPEC)){
        n->type(make_primitive_type(4,cdk::TYPE_UNSPEC));
      }
    ...
    if (_function->is_typed(cdk::TYPE_INT)) {
      ...
      if(node->retval()->is_typed(cdk::TYPE_UNSPEC))
        ...
        if (!node->retval()->is_typed(cdk::TYPE_POINTER))
          throw std::string("wrong type for return. String expected.");
        checkPointersTypes(cdk::reference_type_cast(_function->type()),cdk::reference_type_cast(node->retval()->type()));
        ...
        if(node->retval()->is_typed(cdk::TYPE_STRUCT))
          checkTupleComps(cdk::structured_type_cast(node->retval()->type()),cdk::structured_type_cast(_function->type()));
        else
          throw std::string("wrong type for return. tuple expected.");
    } else {
...
void og::type_checker::do_function_definition_node(og::function_definition_node *const node, int lvl) {
  ...
  // if(node->block()){
  //     auto b = dynamic_cast<og::block_node*>(node->block());
  //     if(b->declarations())
...
void og::type_checker::do_variable_declaration_node(og::variable_declaration_node *const node, int lvl) { 
  ...
  if (node->initializer() != nullptr) {
    ...
    if (node->is_typed(cdk::TYPE_INT)) {
        ...
        if ( node->identifiers().size() != 1 && node->identifiers().size() != cdk::structured_type_cast(node->initializer()->type())->length()) {
    ...
    if (node->is_typed(cdk::TYPE_STRUCT)) { //tuple
      ...
      int size =  node->identifiers().size();
      
      if ( size == 1 ) {
        
        id = node->identifiers().at(0);
        std::shared_ptr<og::symbol> symbol = std::make_shared<og::symbol>(
          node->initializer()->type(),
          id, false,
          (bool)node->initializer(),node->qualifier(), false);
        ...
        for (int j = 0; j < size ; j++ ) {
            ...
            id = node->identifiers().at(j);
            
            std::shared_ptr<og::symbol> symbol = std::make_shared<og::symbol>(
            cdk::structured_type_cast(node->initializer()->type())->component(j),
            id, false,
            (bool)node->initializer(),node->qualifier(), false);
        ...
        id = node->identifiers().at(0); //ONLY 1
...
void og::type_checker::do_tuple_index_node(og::tuple_index_node *const node, int lvl) {
  ...
  auto ret = t->component(node->index() - 1);
  node->type(ret);
} 
...
void og::type_checker::checkTupleComps(std::shared_ptr<cdk::structured_type> tup,std::shared_ptr<cdk::structured_type> fun){
  for(size_t i = 0; i < tup->length(); i++){
    if(fun->component(i)->name() == cdk::TYPE_DOUBLE && tup->component(i)->name() == cdk::TYPE_INT)
      continue;

    if(fun->component(i)->name() != tup->component(i)->name())
      throw std::string("tuple types differ on return");

    if(fun->component(i)->name() == cdk::TYPE_STRUCT)
      checkTupleComps(std::dynamic_pointer_cast<cdk::structured_type>(tup->component(i)),std::dynamic_pointer_cast<cdk::structured_type>(fun->component(i)));
  }
}
